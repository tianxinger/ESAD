...
#新加的损失函数
def loss_ssim(x1, x2, reduce=None):

    ssim = tgm.losses.SSIM(11, reduction='none')
...
class MSGMSLoss(Module):
    ...
    """
    The class for MSGMS Loss imported from GitHub.
    URL: https://github.com/taikiinoue45/RIAD/blob/main/riad/criterions/msgms.py
    User: Taiki Inoue
    Repo. name: RIAD
    """
...
class an_score(Module):
    ...
    def forward(self, img1: Tensor, img2: Tensor, as_loss: bool = True) -> Tensor:
        ...
        for scale in range(self.num_scales):
            ...
            map = self.l1_loss(img1, img2)
            m_map += F.interpolate(map, size=(h, w), mode="bilinear", align_corners=False)
        return torch.mean(m_map, axis=1).unsqueeze(1)
